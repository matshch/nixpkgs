  ...
  bazelBootstrap = stdenv.mkDerivation rec {
    ...
    src =
      if stdenv.hostPlatform.system == "x86_64-linux" then
        ...
        (throw "Unsupported system by bazelBootstrap: ${stdenv.hostPlatform.system}");
  ...
  bazelDeps =
    ...
    stdenv.mkDerivation {
      ...
      buildPhase = ''
        ...
        # symlink to $TMPDIR
        rm -f vendor_dir/rules_jvm_external~~maven~maven/imported_maven_install.json
      ...
      outputHash =
        if stdenv.hostPlatform.system == "x86_64-linux" then
          "sha256-ZSrei+juyVx/DXZgXeaNwAkVaBVwQroD83fW7CP8o68="
        else if stdenv.hostPlatform.system == "aarch64-linux" then
          "sha256-BnSrr1un2BR6gfaVEhOukUa5JoNevbOxQkPmc0IbW6A="
        else if stdenv.hostPlatform.system == "aarch64-darwin" then
          "sha256-RCZj/YPcLn6VJEPICLaS9/QwIRHzGlmPuv9J3+szqIM="
        else if stdenv.hostPlatform.system == "x86_64-darwin" then
          "sha256-uernlVz24oPDVywH7Shj45RQF+kZD022HVaXm0Uzboo="
        else
          (throw "Unsupported system by bazelDeps: ${stdenv.hostPlatform.system}");
